...
from limpeza_dados import df_dropna
...
df['idade_ajustada'] -= ((data.atual.month <= df['data_atualizada'].dt.month) & (data_atual.day < df['data_atualizada'].dt.day)).astype(int)
...
df['estado_sigla'] = df[df['estado_sigla'].str.upper().apply(lambda x: x if x in estados_br else 'Desconhecido')
